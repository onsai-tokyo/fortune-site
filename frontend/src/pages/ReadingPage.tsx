...
export default function ReadingPage() {
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      let state = await fetch('/api/reading/status', { headers: authHeaders() }).then(r => r.json())
      ...
      const historyBody = await fetch('/api/reading/conversations', { headers: authHeaders() }).then(r => r.json())
    ...
    try {
      ...
      while (true) {
        ...
        for (const line of decoder.decode(value, { stream: true }).split('\n')) {
          ...
          try { const part = JSON.parse(raw); if (part.delta?.text) { answer += part.delta.text; setMessages(prev => [...prev.slice(0, -1), { role: 'assistant', content: answer }]) } } catch { /* incomplete SSE */ }
        }
      }
  ...
  return <main className="min-h-screen bg-[#faf7ef] text-[#211d18] px-4 py-10 font-serif"><div className="max-w-3xl mx-auto">
    ...
    {status?.remaining === 0 && !status.premium ? <section className="border border-[#c8aa6d] bg-[#fffaf0] p-6 rounded-2xl"><h2 className="text-xl">もう少し、深く読み解きますか。</h2><p className="mt-3 leading-7 text-[#685e53]">無料鑑定の続きとして、気になったことを何度でも質問できます。月額料金・自動更新・解約方法は決済画面と特定商取引法の表記で確認できます。</p><button onClick={checkout} className="w-full mt-5 py-4 bg-[#9a6d16] text-white rounded-lg">鑑定を続ける</button></section> : <div className="sticky bottom-3 bg-[#fffdf8] border border-[#d8c79e] shadow-xl rounded-2xl p-3 flex gap-2"><textarea value={input} onChange={e => setInput(e.target.value)} placeholder="鑑定結果について質問する…" className="flex-1 bg-transparent px-3 py-2 resize-none outline-none" rows={2} /><button onClick={() => send()} disabled={!input.trim() || sending} className="px-5 rounded-xl bg-[#9a6d16] text-white disabled:opacity-40">送信</button></div>}
