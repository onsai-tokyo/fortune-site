"""Disposable local Postgres only. Requires D02_CHAT_PG_BIN; never reads DB URLs."""
import concurrent.futures, json, os, pathlib, subprocess, tempfile

base = pathlib.Path(__file__).resolve().parents[1]
binpath = pathlib.Path(os.environ['D02_CHAT_PG_BIN']).resolve()
psql = os.environ.get('D01_PSQL', '/opt/homebrew/opt/libpq/bin/psql')
root = pathlib.Path(tempfile.mkdtemp(prefix='fatelab-d02-chat-', dir='/private/tmp'))
...
try:
    ...
    sql((base/'chat_creation_d02.sql').read_text())
    def chat(op=70, n=1, question='first question', source=None):
        return json.loads(owner(f"select create_reading_chat('{uid(op)}','{source or first['id']}',{literal(question)},'chat');",n))
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as pool:
        created=list(pool.map(lambda _:chat(),range(6)))
    expect(len(set(x['id'] for x in created))==1 and sum(not x['reused'] for x in created)==1,'chat: six concurrent requests create one conversation')
    child=created[0]['id']
    expect(chat()['id']==child,'chat: lost acknowledgement returns same ID')
    expect(chat(question='changed')['state']=='conflict','chat: same operation changed question conflicts')
    expect(chat(71,2)['state']=='not_found','chat: foreign source rejected')
    expect(owner(f"select report_text='v1 body' and reading_revision_id='{first['revisionId']}' from reading_conversations where id='{child}';")=='t','chat: source body and revision pinned')
    owner(f"delete from reading_conversations where id='{child}';")
    expect(chat()['state']=='deleted','chat: deleted conversation cannot be recreated by old operation')
    new=chat(72)
    expect(new['id']!=child,'chat: new explicit operation can create another conversation')
    count=sql('select count(*) from reading_conversations;')
    sql("create function fail_chat_test() returns trigger language plpgsql as $$ begin raise exception 'synthetic_operation_failure'; end $$; create trigger fail_chat_test before insert on reading_chat_operations for each row execute function fail_chat_test();")
    try: chat(73)
    except subprocess.CalledProcessError: pass
    else: raise AssertionError('chat operation failure not injected')
    expect(sql('select count(*) from reading_conversations;')==count,'chat: operation insert failure rolls back conversation insert')
    expect(sql(f"select count(*) from reading_chat_operations where op_id='{uid(73)}';")=='0','chat: failed transaction leaves no operation')
    expect(sql("select has_table_privilege('authenticated','reading_chat_operations','update');")=='f','chat: client cannot tamper with operation ledger')
    expect(sql("select has_function_privilege('anon','create_reading_chat(uuid,uuid,text,text)','execute');")=='f','chat: anonymous execution rejected')
finally:
