...
import { buildSynastryFacts, computeAmbitionAlignmentProfile, computeCompatibilityProfile, computeEgoCompetitionProfile, computeFateCompanionFeelingProfile, computeLongTermBindingProfile, computeMysteryDistanceProfile, computeMutualUnderstanding, computePartnershipTeamFeelingProfile, computePowerBalanceProfile, computePrivateAffectionProfile, computeRelationScores, computeRelationshipBoredomRiskProfile, computeRelationshipStimulationNeedProfile, computeSocialDisplayAffectionProfile, computeTransparencyProfile, computeTrustStabilityProfile, type CompatibilityProfileScore, type MutualUnderstandingProfile, type RelationAxis, type RelationScore } from './synastryFacts.js'
...
export function buildCompatibilityTraitScoreBundle(self: ReportInput, partner: ReportInput): CompatibilityTraitScoreBundle {
  ...
  const transparency = computeTransparencyProfile(selfScores, partnerScores)
  const profile = [
    ...computeCompatibilityProfile(synastry, { self: Boolean(self.birthTime), partner: Boolean(partner.birthTime) }),
    computeEgoCompetitionProfile(selfScores, partnerScores),
    computeAmbitionAlignmentProfile(selfScores, partnerScores),
    computePowerBalanceProfile(selfScores, partnerScores),
    transparency,
    computeMysteryDistanceProfile(synastry, transparency),
    computeSocialDisplayAffectionProfile(selfScores, partnerScores),
    computePrivateAffectionProfile(selfScores, partnerScores),
  ]
...
function effectiveRelationScores(id: string, context: PairContext): RelationScore[] {
  ...
  return context.relationScores.map(relation => {
    ...
    const profileKey = relation.key === 'communication' ? 'conversational_flow'
      : relation.key === 'depth' ? 'conversational_depth'
...
function pagesFor(id: string, context: PairContext, resolvedAxis?: RelationAxis): ReportCardPage[] {
  ...
  const emotionalBlock = id === 'compat-attraction'
    ...
    : null
  const admirationBlock = id === 'compat-attraction'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'admiration_mutual'), item.cue)
    : null
  ...
  const friendshipBlock = id === 'compat-beginning'
    ...
    : null
  const domesticBlock = id === 'compat-marriage'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'domestic_compatibility'), item.cue)
    : null
  ...
  const safetyBlock = id === 'compat-caution'
    ...
    : null
  const dependencyBlock = id === 'compat-caution'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'dependency_intensity'), item.cue)
    : null
  const trustBlock = id === 'compat-caution'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'trust_stability'), item.cue)
    : null
  const transparencyBlock = id === 'compat-caution'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'transparency'), item.cue)
    : null
  const mysteryDistanceBlock = id === 'compat-caution'
    ...
    : null
  const socialDisplayAffectionBlock = id === 'compat-caution'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'social_display_affection'), item.cue)
    : null
  const privateAffectionBlock = id === 'compat-caution'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'private_affection'), item.cue)
    : null
  ...
  const prideBlock = id === 'compat-friction'
    ...
    : null
  const egoCompetitionBlock = id === 'compat-friction'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'ego_competition'), item.cue)
    : null
  const powerBalanceBlock = id === 'compat-friction'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'power_balance'), item.cue)
    : null
  const conflictFrequencyBlock = id === 'compat-friction'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'conflict_frequency'), item.cue)
    : null
  const growthBlock = id === 'compat-growth'
    ...
    : null
  const noveltyBlock = id === 'compat-growth'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'novelty_compatibility'), item.cue)
    : null
  const sharedProjectBlock = id === 'compat-growth'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'shared_project_compatibility'), item.cue)
    : null
  const ambitionBlock = id === 'compat-growth'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'ambition_alignment'), item.cue)
    : null
  const teamFeelingBlock = id === 'compat-growth'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'partnership_team_feeling'), item.cue)
    : null
  const adventureBlock = id === 'compat-growth'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'adventure_compatibility'), item.cue)
    : null
  const stimulationBlock = id === 'compat-growth'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'relationship_stimulation_need'), item.cue)
    : null
  const boredomBlock = id === 'compat-growth'
    ? compatibilityProfileBlock(context.compatibilityProfile.find(score => score.key === 'relationship_boredom_risk'), item.cue)
    : null
