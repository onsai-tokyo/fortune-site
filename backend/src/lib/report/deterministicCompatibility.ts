...
import { buildSynastryFacts, computeAmbitionAlignmentProfile, computeCompatibilityProfile, computeEgoCompetitionProfile, computeFateCompanionFeelingProfile, computeLongTermBindingProfile, computeMysteryDistanceProfile, computeMutualUnderstanding, computePartnershipTeamFeelingProfile, computePowerBalanceProfile, computeRelationScores, computeRelationshipBoredomRiskProfile, computeRelationshipStimulationNeedProfile, computeSocialDisplayAffectionProfile, computeTransparencyProfile, computeTrustStabilityProfile, type CompatibilityProfileScore, type MutualUnderstandingProfile, type RelationAxis, type RelationScore } from './synastryFacts.js'
...
export function buildDeterministicCompatibilityReport(self: unknown, partner: unknown, relationshipType: RelationshipType, relationshipLabel: string, scoreBundle?: CompatibilityTraitScoreBundle): StructuredReport {
  ...
  const cards = selected.map(([id, title]) => {
    ...
    const chapterProfileKeys: CompatibilityProfileScore['key'][] = id === 'compat-repair'
      ...
      : id === 'compat-caution' ? ['emotional_intimacy', 'emotional_safety', 'mystery_distance', 'social_display_affection', 'transparency', 'trust_stability', 'dependency_intensity', 'conversational_depth']
