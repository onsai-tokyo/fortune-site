...
import { buildSynastryFacts, computeCompatibilityProfile, computeEgoCompetitionProfile, computeMutualUnderstanding, computeRelationScores, type CompatibilityProfileScore, type MutualUnderstandingProfile, type RelationAxis, type RelationScore } from './synastryFacts.js'
...
export function buildDeterministicCompatibilityReport(self: unknown, partner: unknown, relationshipType: RelationshipType, relationshipLabel: string, scoreBundle?: CompatibilityTraitScoreBundle): StructuredReport {
  ...
  const cards = selected.map(([id, title]) => {
    ...
    const chapterProfileKeys: CompatibilityProfileScore['key'][] = id === 'compat-repair'
      ...
      : id === 'compat-friction' ? ['conflict_intensity', 'pride_collision', 'ego_competition', 'repair_capacity', 'emotional_safety', 'conversational_flow']
