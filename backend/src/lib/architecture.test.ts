...
test('StoreKitは同期検知だけに使いプレミアム表示はサーバー状態を正とする', () => {
  ...
  assert.match(purchases, /deliver\(transaction[\s\S]{0,300}status = try await APIClient\.shared\.status/)
...
test('ログイン後の初期表示は非表示エラーやStoreKit同期でloadingに固定されない', () => {
  const root = read('ios/FateLab/RootView.swift')
  assert.match(root, /catch \{\s+guard !Task\.isCancelled, owner\.isCurrent\(auth\) else \{ return \}\s+if case \.returning = landingState \{ return \}\s+landingState = \.failed/)
  assert.doesNotMatch(root, /guard userFacingMessage\(error\) != nil else \{ return \}/)
})

test('新規ユーザーは出生情報の入力後にあなたタブで鑑定を自動開始する', () => {
  const root = read('ios/FateLab/RootView.swift')
  assert.match(root, /OnboardingView\(userID: userID\) \{ input in\s+pendingInput = input\s+tabRouter\.openYourReading\(\)\s+onboardedUserID =/)
...
test('PR12の鑑定APIは認証トークンを送り、チャット本文と次質問を分離する', () => {
  ...
  assert.match(api, /transport\.bytes/)
...
test('鑑定書保存は操作IDと不変revisionを使いサーバー生成タイトルを持つ', () => {
  const api = read('ios/FateLab/APIClient.swift')
  const reading = read('backend/src/routes/reading.ts')
  assert.match(api, /snapshot\.operationID\.uuidString/)
  ...
  assert.match(reading, /saveReadingSnapshot/)
  assert.doesNotMatch(reading, /refreshExisting/)
})

// Auth error privacy, transport failure and session persistence now run against
// the pinned Swift SDK in ios/FateLabTests/AuthTests.swift, rather than text matching.
...
test('UI刷新は白黒テーマ・段階式オンボーディング・認証ゲートを維持する', () => {
  ...
  assert.match(onboarding, /AccountStorage\.key\("onboarding\.draft"/)
...
test('相性鑑定は明示された自己鑑定IDを検証し、チャットは逐次SSEを返す', () => {
  ...
  assert.match(api, /transport\.bytes\(for: call\)/)
...
test('相性鑑定は凪等な会話を1件保存し完了イベントでIDを返す', () => {
  ...
  assert.match(partners, /complete_compatibility_operation/)
  assert.match(partners, /'compatibility', partner\.id/)
...
test('相性鑑定はpartnersの一経路だけで生成し同じ課金判定を通る', () => {
  ...
  const api = read('ios/FateLab/APIClient.swift')
  assert.match(partners, /begin_compatibility_operation/)
  assert.match(partners, /fail_compatibility_operation/)
  assert.doesNotMatch(partners, /requirePoints\(3\)|await addPoints/)
  ...
  assert.match(api, /case 402: throw APIError\.paymentRequired/)
...
test('相性結果は選択画面と分離し本人鑑定と同じカード導線を使う', () => {
  ...
  assert.match(api, /value\["conversationId"\] = conversationID/)
...
test('ログイン後は保存済み鑑定を優先し未鑑定ユーザーだけ本人別オンボーディングを出す', () => {
  const root = read('ios/FateLab/RootView.swift')
  const reading = read('backend/src/routes/reading.ts')
  assert.match(root, /AccountStorage\.key\("onboarding\.completedUserID"/)
...
test('基本鑑定は一種類だけ表示し質問は独立したチャット履歴へ保存する', () => {
  ...
  assert.match(reading, /relation\?\.reading_id/)
  assert.match(reading, /readingRouter\.post\('\/conversations\/:id\/chat'/)
  const chatSQL = read('backend/supabase/chat_creation_d02.sql')
  assert.match(reading, /rpc\('create_reading_chat'/)
  assert.match(chatSQL, /source\.reading_revision_id/)
  assert.match(chatSQL, /'_sourceKind',source\.kind/)
  assert.match(reading, /対話の保存状況を確認できませんでした/)
...
test('App Store同期は別アカウントを除外し明示的なSandbox復元だけ移管する', () => {
  ...
  const purchases = read('ios/FateLab/PurchaseManager.swift')
  const settings = read('ios/FateLab/SettingsView.swift')
  const sql = read('backend/supabase/apple_event_reconciliation_b02.sql')
  assert.match(apple, /receiveAndApply/)
  assert.doesNotMatch(apple, /from\('app_store_subscriptions'\)/)
  assert.match(sql, /pg_advisory_xact_lock/)
  assert.match(sql, /sandbox_production_conflict/)
  assert.match(sql, /ownership_sequence/)
  assert.match(apple, /res\.json\(\{ verified: true, skipped: true/)
  assert.match(purchases, /再購入せず、購入状況の確認を再試行/)
  ...
  assert.match(purchases, /allowTransfer: true/)
