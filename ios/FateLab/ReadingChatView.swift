...
struct ReadingChatView: View {
            ...
            ScrollViewReader { proxy in
                ScrollView {
                    LazyVStack(alignment: .leading, spacing: 18) {
                        ...
                        if messages.last?.role == "assistant" && !isBlocked {
            ...
            VStack(spacing: 8) {
                ...
                HStack(alignment: .bottom, spacing: 10) {
                    ...
                    Button("読み解く") {
                        ...
                        .foregroundStyle(.white).padding(.horizontal, 15).frame(height: 38)
                        .background(FateTheme.gold).clipShape(RoundedRectangle(cornerRadius: 8))
    ...
    private let followUpSuggestions = ["この流れに向けて準備しておくことは？", "仕事と恋愛が重なる時期の考え方は？", "次の大きな転換期はいつごろ？"]
    ...
    private func messageBubble(_ message: ReadingMessage) -> some View {
        HStack {
            ...
            Text(message.content).font(.system(size: 16)).lineSpacing(7).padding(16)
                .background(message.role == "user" ? FateTheme.gold.opacity(0.14) : FateTheme.paper)
                .overlay(RoundedRectangle(cornerRadius: 16).stroke(FateTheme.line.opacity(0.8)))
    ...
    private func load() async {
        ...
        do {
            let value = try await APIClient.shared.conversation(id: conversationID, token: token)
    ...
    private func loadStatus() async {
        guard let token = auth.session?.accessToken else { return }
        status = try? await APIClient.shared.status(token: token)
    }
    ...
    private func send() async {
        ...
        do {
            let answer = try await APIClient.shared.ask(conversationID: conversationID, question: question, token: token)
            messages.append(ReadingMessage(id: nil, role: "assistant", content: answer, createdAt: nil))
            await loadStatus()
        } catch {
                ...
                VStack(alignment: .leading, spacing: 22) {
                    ...
                    if let session = auth.session {
                        if purchases.product == nil, purchases.errorMessage != nil {
                            ...
                            Button("継続鑑定を始める") {
                                Task { await purchases.purchase(userID: session.user.id, accessToken: session.accessToken); onRefresh() }
                            }.buttonStyle(GoldButtonStyle()).disabled(purchases.product == nil || purchases.isWorking)
                        }
                        Button("購入を復元") {
                            Task { await purchases.restore(accessToken: session.accessToken); onRefresh() }
